...
namespace PlatCDemo_App
{
    class BLE
    {
        ...
        private BLEConnectionStateChangeCb_t g_activityBLEConnStateChangeCb = null;

        // Delegates
        public delegate void BLEConnectionStateChangeCb_t(ConnectionState state);
        ...
        public void RegisterBLEConnectionStateChangeCb(BLEConnectionStateChangeCb_t activityCb)
        {
            g_activityBLEConnStateChangeCb = activityCb;
        }
        public ConnectionState GetConnectionState()
        {
            if(null != g_BLEAdapterObj_connectionGattServer)
            {
                return g_BLEAdapterObj_connectionGattServer.State;
            }

            return ConnectionState.Disconnected;
        }
        public async Task<bool> BLE_Scan()
        {
            ...
            if (lm.IsProviderEnabled(LocationManager.GpsProvider) && lm.IsProviderEnabled(LocationManager.NetworkProvider))
            {
                ...
                await g_BLEAdapterObj.ScanForBroadcasts(Observer.Create<IBlePeripheral>(g_BLEAdapterObj_scanResponsesObserver), TimeSpan.FromSeconds(5));
        ...
        public async Task<bool> BLE_ScanAndConnect(string deviceName)
        {
            ...
            {
                /* Scan for device */
                if(true == await BLE_Scan())
                {
                    /* Connect to expected device name */
                    if(true == g_BLEScanDevices.ContainsKey(deviceName))
                        return await BLE_Connect(g_BLEScanDevices[deviceName]);
                }
            }

            return false;
        }
        public async Task<bool> BLE_Disconnect()
        {
            if (null != g_BLEAdapterObj_connectionGattServer)
            {
                if ((ConnectionState.Connected == g_BLEAdapterObj_connectionGattServer.State) || (ConnectionState.Connecting == g_BLEAdapterObj_connectionGattServer.State))
                {
                    await g_BLEAdapterObj_connectionGattServer.Disconnect();
                }
            }
        ...
        internal Task BLE_WriteTargetCharacteristic(object p)
        {
