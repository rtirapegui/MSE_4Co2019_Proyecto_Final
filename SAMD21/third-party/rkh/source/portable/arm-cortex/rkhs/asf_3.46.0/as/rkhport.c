...
/* ---------------------------- Local data types --------------------------- */
static volatile ruint	g_cpu_irq_critical_section_counter;
static volatile rbool_t g_cpu_irq_prev_interrupt_state;
...
{
	if (g_cpu_irq_critical_section_counter == 0) {
		if (cpu_irq_is_enabled()) {
			cpu_irq_disable();
			g_cpu_irq_prev_interrupt_state = true;
			} else {
			/* Make sure the to save the prev state as false */
			g_cpu_irq_prev_interrupt_state = false;
		}

	}

	g_cpu_irq_critical_section_counter++;
}

void rkhport_exit_critical(void)
{
	/* Check if the user is trying to leave a critical section when not in a critical section */
	Assert(g_cpu_irq_critical_section_counter > 0);

	g_cpu_irq_critical_section_counter--;

	/* Only enable global interrupts when the counter reaches 0 and the state of the global interrupt flag
	   was enabled when entering critical state */
	if ((g_cpu_irq_critical_section_counter == 0) && (g_cpu_irq_prev_interrupt_state)) {
		cpu_irq_enable();
	}
}
